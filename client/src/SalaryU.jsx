...
function SalaryU() {
    ...
    useEffect(() => {
        axios.get('http://localhost:4000/Salary/getSalary')  
            .then(response => {
                ...
                setFilteredSalaries(response.data); // Initialize filteredSalaries with all salaries
            })
            .catch(error => {
        ...
        if (confirmDelete) {
            axios.delete(`http://localhost:4000/Salary/deleteSal/${id}`)
                .then(res => {
                    ...
                    setFilteredSalaries(filteredSalaries.filter(item => item._id !== id)); // Update filteredSalaries as well
                })
    ...
    const handleChange = (e) => {
        ...
        if (e.target.value === "") {
            setFilteredSalaries(salary); // Reset filteredSalaries to all salaries when searchTerm is empty
        } else {
